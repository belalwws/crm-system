                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <Dropdown
                      items={[
                        ...
                        { label: "Delete", icon: <Trash2 className="w-4 h-4" />, onClick: () => onDelete(deal.id || deal._id), danger: true },
                      ]}
                    />
                  </div>
                  {deal.customer && (
...
export default function DealsPage() {
  ...
  const openEditModal = (deal: Deal) => {
    setFormData({
      ...
      customerId: deal.customer?.id || deal.customer?._id || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = await getToken();
      const url = isEditing
        ? `${process.env.NEXT_PUBLIC_API_URL}/deals/${selectedDeal?.id || selectedDeal?._id}`
              ...
              <div className="mt-4 pt-3 border-t border-neutral-800 flex justify-between items-center">
                ...
                <Dropdown
                  items={[
                    ...
                    { label: "Delete", icon: <Trash2 className="w-4 h-4" />, onClick: () => handleDelete(deal.id || deal._id), danger: true },
                  ]}
                />
              </div>
            </Card>
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {stages.map((stage) => (
                  <button
                    key={stage.value}
                    onClick={() => {
                      handleStageChange(selectedDeal.id || selectedDeal._id, stage.value);
                      setShowViewModal(false);
                    }}
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                onClick={() => {
                  setShowViewModal(false);
                  handleDelete(selectedDeal.id || selectedDeal._id);
                }}
                className="flex-1"
