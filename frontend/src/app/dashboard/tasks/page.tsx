...
interface Task {
  ...
  status: string;
  dueDate?: string;
  customer?: { id: string; _id: string; name: string };
  deal?: { id: string; _id: string; title: string };
  createdAt: string;
}
    ...
    >
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <Dropdown
                  items={[
                    ...
                    { label: "Delete", icon: <Trash2 className="w-4 h-4" />, onClick: () => onDelete(task.id || task._id), danger: true },
                  ]}
                />
              </div>
            </div>
      ...
      <div className="border-t border-neutral-800 px-4 py-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        {statuses.map((status) => (
          <button
            key={status.value}
            onClick={() => onStatusChange(task.id || task._id, status.value)}
...
export default function TasksPage() {
  ...
  const openEditModal = (task: Task) => {
    setFormData({
      ...
      customerId: task.customer?.id || task.customer?._id || "",
      dealId: task.deal?.id || task.deal?._id || "",
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = await getToken();
      const url = isEditing
        ? `${process.env.NEXT_PUBLIC_API_URL}/tasks/${selectedTask?.id || selectedTask?._id}`
      ...
      {tasks.length === 0 ? (
        ...
        <div className="space-y-3 animate-slide-up">
          {filteredTasks.map((task, index) => (
            <div key={task.id || task._id} className="animate-fade-in" style={{ animationDelay: `${index * 0.03}s` }}>
              <TaskCard
                task={task}
